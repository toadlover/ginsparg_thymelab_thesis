...
for index in range(len(pdb_list)):
	...
	if (len(mini_list) % 100) == 0:
		...
		for item in mini_list:
			...
			#run auto_params
			auto_params_job_writer.write("./auto_params.sh " + item + ".pdb\n")
			#move all files for the pdb to the folder
...
for pdb in pdb_list:
	...
	for file in single_ligand_files:
		...
		flag_writer.write("-water_score_cutoff -0.3\n")
	...
	for file in single_ligand_files:
		...
		#run ligand_motifs
		job_writer.write("/data/project/thymelab/running_Rosetta/ari_work/Rosetta_Code_Fresh/main/source/bin/identify_ligand_motifs.linuxgccrelease @" + flag_file_name + "\n")
		...
		job_writer.write("mv AllMattMotifsFile.motifs " + file_name + "\n")
	...
	os.system("squeue -A abgvg9 | wc -l  > squeue_file.txt")
